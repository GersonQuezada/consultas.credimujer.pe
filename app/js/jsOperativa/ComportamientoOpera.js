...
$(document).on("click", "#Btn_Inserta_Socia_ManualBN", function (e){     
    ...
    if(CODREGION ==0){
      Swal.fire({type:'error',title : 'Error!!',text: 'Debe Seleccionar una region'})
    }else if (nombre.length == ""){
      Swal.fire({type:'error',title:'Error!!',text : 'Debe ingresar un Nombre'})
    }else{
      ...
      "language": {
       ...
       columns:
        [
                {"data" : "Add",
                  render: function ( data, type, row) {           
                  var botonAgregar = '<button type="button" id="BotonAgregarTabla" class="BotonAgregarTabla" onclick = "AbrirModalInsertBn(\''+row.TIPOREGISTRO+'___'+row.FECHAPROCEBN+'___'+row.TIPODESEMBOLSO+'___'+row.NROCREPERSONA+'___'+row.NUMCREDITO+'___'+row.CODSOCIA+'___'+row.APELLIDOSNOMBRES+'___'+row.NRODNI+'___'+row.NROCICLO+'___'+row.CODREGION+'___'+row.CODMONEDA+'___'+row.CODENTIFINAN+'___'+row.TIPOCREDITO+'___'+row.MTOAPROBADO+'___'+row.MTODESEMBOLSADO+'___'+row.SITUACION+'___'+row.NROSOLICITUD+'___'+row.FECHAREGISTRO+'___'+row.FECHACREDITO+'___'+row.CODASOCIACION+'___'+row.DESASOCIACION+'___'+row.NROSOCIO+'___'+row.CARGATABLA+'\');" id = "BotonEditar"><i class="fas fa-user-plus"></i></button>'; 
                  return botonAgregar;     
                  }
                },
        ...
        "language": {
         ...
         columns:
          [
                  {"data" : "Add",
                    render: function ( data, type, row) {           
                    var botonAgregar = '<button type="button" id="BotonAgregarTabla" class="BotonAgregarTabla" onclick = "IsertarSociaAnulada(\''+row.FECHAPROCEBN+'___'+row.NRODNI+'___'+row.CODREGION+'\');" id = "BotonEditar"><i class="fas fa-user-plus"></i></button>'; 
                    return botonAgregar;     
                    }
                  }, 
                  {"data" : "FECHAPROCEBN"}, 
